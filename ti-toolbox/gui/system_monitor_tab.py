...
class ProcessMonitorThread(QThread):
    ...
    def __init__(self):
        ...
        self.relevant_keywords = [
            ...
            'subject_atlas', 'mri_convert', 'mris_', 'mri_', 'fs_',
            'preprocessor', 'field_extract', 'mesh2nii'
        ]
    ...
    def is_relevant_process(self, proc_name, cmdline):
        ...
        if 'python' in search_text and any(term in search_text for term in ['ti-', 'gui/', 'pre-process/', 'simulator/', 'flex-search/', 'ex-search/']):
            return True
...
class SystemMonitorTab(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        self.process_table.setHorizontalHeaderLabels([
            "PID", "Process Name", "Command", "CPU %", "Memory %", "Memory (MB)", "Runtime"
        ])
    ...
    def manual_refresh(self):
        ...
        if self.monitor_thread and self.monitor_thread.isRunning():
            ...
            QtCore.QTimer.singleShot(1000, lambda: self.status_label.setText("Monitoring active..."))
    ...
    def update_system_stats(self, stats):
        ...
        if stats['memory_total'] > 0:
            ...
            self.memory_label.setText(f"Memory: {stats['memory_percent']:.1f}% ({memory_gb:.1f}/{total_gb:.1f} GB)")
        else:
    ...
    def update_process_table(self, processes):
        ...
        for row, proc in enumerate(processes):
            # PID
            self.process_table.setItem(row, 0, QtWidgets.QTableWidgetItem(str(proc['pid'])))
            ...
            if proc['cpu_percent'] > 50:
                cpu_item.setBackground(QtGui.QColor(255, 200, 200))  # Light red for high CPU
            ...
            if proc['memory_percent'] > 10:
                mem_item.setBackground(QtGui.QColor(255, 200, 200))  # Light red for high memory
            self.process_table.setItem(row, 4, mem_item)
            
            # Memory MB
            self.process_table.setItem(row, 5, QtWidgets.QTableWidgetItem(f"{proc['memory_mb']:.1f}"))
            
            # Runtime
            self.process_table.setItem(row, 6, QtWidgets.QTableWidgetItem(proc['runtime']))
    ...
    def terminate_selected_process(self):
        ...
        if current_row < 0:
            QtWidgets.QMessageBox.warning(self, "No Selection", "Please select a process to terminate.")
            return
